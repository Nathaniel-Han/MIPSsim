...
int main(int argc, char** argv)
{
    ...
    //simulation
    pipeline_Console console(INITIAL_COM_ADDR, data_addr_start/4,data_addr_end/4, OUTPUT_SIM);
    ...
    console.Instructions.clear();
    
    int issue_pre_array[PRE_ISSUE_SIZE];
    memset(issue_pre_array, -1, sizeof(int)*PRE_ISSUE_SIZE);
    
    while (!console.isFinished) {
        console.process(disassembler);
        console.simu_print();
        console.cycle_count++;
    }
}
